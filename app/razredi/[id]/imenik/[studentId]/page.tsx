            ...
            return (
              <div key={element} className="flex justify-between items-center p-3 border-b border-gray-200">
                ...
                <div className="flex gap-2 flex-1 justify-end">
                  {/* Render actual grades */}
                  {elementGrades.map((g, i) => (
                    <div 
                      key={g.id} 
                      className={`w-8 h-8 flex items-center justify-center font-bold text-sm cursor-pointer ${g.is_written ? 'bg-gray-100 border border-gray-300' : 'bg-white border border-gray-200'}`}
                      title={g.note}
                    >
                      {g.grade}
                    </div>
                  ))}
                  {/* Empty slots for new grades */}
                  {Array.from({length: 10 - elementGrades.length}).map((_, i) => (
                    <div 
                      key={`empty-${i}`} 
